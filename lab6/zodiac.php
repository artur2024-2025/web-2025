...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($dmY !== null) {
        [$d, $m, $y] = $dmY;
        $sign = zodiac_sign($d, $m);
        $result = sprintf('%02d.%02d.%04d → %s', $d, $m, $y, $sign);
    }
}
?>
<!doctype html>
...
<head>
  ...
  <style>
    ...
    input[type=text]{padding:10px 12px;border:1px solid #cfd3d8;border-radius:8px;width:180px}
...
<div class="wrap">
  ...
  <p class="note">Формат ввода: <b>ДД.ММ.ГГГГ</b> (пример: <code>15.04.1452</code>).</p>

  <form method="post" action="">
    <label for="date">Дата:</label>
    <input type="text" id="date" name="date" placeholder="ДД.ММ.ГГГГ" value="<?= h($raw ?? '') ?>">
